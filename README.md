...
- **🎯 默认零后台** - 安装后只有 hosts 过滤在工作，所有可选功能默认关闭
- **🚀 低耗电设计** - 当保护、自动刷新、拦截历史与 DoH 都关闭时，模块保持零额外工作状态
- **🎨 现代化 WebUI** - 重新设计的应用式控制台，五个页签：首页 / 规则 / 应用 / 日志 / 设置
- **🔒 隐私优先** - 不联网直到你确认使用说明，不写入 Android Private DNS
- **📦 轻量安装** - 无需独立 App，运行在模块管理器内置 WebUI 中
...
本项目以 **GPL-3.0-only** 授权，完整条款见 [LICENSE](LICENSE)，版权声明与第三方清单见 [NOTICES](NOTICES)。
